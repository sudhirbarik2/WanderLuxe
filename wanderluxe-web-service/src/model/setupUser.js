...
let userData = [
    { userId: "U1001", name: "abc", emailId: "abc@gmail.com", contactNo: 1234567890, password: "Abc@1234", bookings: ["B1001", "B1002"] },
...
let destinationsData = [
    {
        "destinationId": "D10017",
        "continent": "Asia",
        "name": "North, East & West Sikkim (India)",
        "imageUrl": "/assets/sikkim.jpg",
        "details": {
            "about": "Spire-studded skylines and fascinating histories are just the beginning.Dotted with mountains and unique local cultures.",
            "itinerary": {
                "dayWiseDetails": {
                    "firstDay": "Board your flight to Bagdogra.",
                    "restDaysSightSeeing": [
                        "Gangtok",
                        "Lachung",
                        "Pelling",
...
exports.userSetup = () => {
    return connection.getUserCollection().then((myCollection) => {
        return myCollection.deleteMany().then(() => {
            return myCollection.insertMany(userData).then(() => {
                //console.log("getUserCollection");
                return connection.getBookingCollection().then((booking) => {
                    //console.log("getBookingCollection");
                    return booking.deleteMany().then(() => {
                        return booking.insertMany(bookingData).then(() => {
                            return connection.getPackageCollection().then((packages) => {
                                //console.log("getPackageCollection");
                                return packages.deleteMany().then(() => {
                                    return packages.insertMany(destinationsData).then(() => {
                                        // console.log(destinationsData);
                                        return connection.getHotDealsCollection().then((hotDeals) => {
                                            return hotDeals.deleteMany().then(() => {
                                                return hotDeals.insertMany(hotdealData).then((data) => {
                                                    if (data) {
